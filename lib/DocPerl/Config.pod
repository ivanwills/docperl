...
This allows you to set general parameters that are used by the templates.
...
Set to a true value this will show the clear cache link which will allow you
to clear the list page cache file so that you can easily see any new modules
on your system.

The down side to allowing this link is that any one can clear the cache which
could be a performance issue for you.
...
static files (such as js & images) from the same location as docperl.cgi.
