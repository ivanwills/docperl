...
DocPerl is straightforward to install.  It consists of wholely of Perl scripts
...
A webserver (eg. Apache, IIS, LightHTTPD etc...)
...
Installation on Windows has improved since the initial realeses. The
...
or your Perl is not installed into F<C:/Perl>). Work-arounds are to change the
bang line from:
