...
download a copy of DocPerl and extract the files. E.g.

 $ tar xzf docperl-v0.9.1.tar.gz

=head1 Setup

Then you should run ./checksetup.pl which tell you what required modules are
installed or not on your system. Also ./checksetup.pl can be used to create
pre-cached POD files to speed up usage and enable full text searching and
function name searching. For for example use the command:

  $ ./checksetup.pl -c pod,text,func
...
modules to be publicly available.
